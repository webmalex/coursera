...
# 
# __Ваш ответ (1 балл)__: Регуляризаторы явлются штрафным слагаемым в функционали который мы минимизируем при обучении, который штрафует большие веса признаков
...
#     * Ответ: **Все веса занулятся.** Так как исходный функционал ошибки становится пренебрежительно мал на фоне регулризатора 
# 1. Можно ли утверждать, что Lasso исключает один из признаков windspeed при любом значении alpha > 0? А Ridge?
#     * Ответ: Нет и для L2 тоже Нет
...
#     * Ответ: Не очень выраженная но видна. Да логично.
...
#     * Ответ: Месяц был исключен так как сильно коррелирует с сезоном но меньше коррелирует с результирующим признаком. Еще близкий к нулю вес исмеет скорость втера в метраз в секунду по той же причине мультиколинеарности. Но при этом нет нет не верно утверждение что эти рпизнаки никак не влияют на спрос.
